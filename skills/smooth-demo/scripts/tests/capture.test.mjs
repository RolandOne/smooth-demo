...
// Codex desktop browser shape: frames are polled through readEvents.
function pollingFake({ width = 1440, fail = false } = {}) {
  let reads = 0;
  let started = false;
...
// Playwright CDPSession shape: frames arrive as push events; a failing ack
// simulates a dropped session.
function pushFake({ width = 1440, fail = false } = {}) {
  ...
  return {
    ...
    async send(method) {
      ...
      if (method === 'Page.startScreencast') {
        setTimeout(() => handlers.get('Page.screencastFrame')?.({ data: png(1440, 900), sessionId: 1 }), 0);
      }
      if (method === 'Page.screencastFrameAck') {
        if (fail) { disconnected.resolve(); throw new Error('Simulated disconnection'); }
        frameAcked.resolve();
      }
      return {};
    },
...
test('rejects a session with neither push nor polling events', () => {
  assert.throws(() => createRecorder({ send() {} }, os.tmpdir()), /on\/off .* or readEvents/);
});

for (const [mode, fake] of [['polling', pollingFake], ['push', pushFake]]) {
  test(`${mode}: rejects cropped preflight before starting screencast and saves diagnosis`, options, async t => {
  ...
  test(`${mode}: interruption preserves frames and durable action journal`, options, async t => {
    const dir = await directory(t);
  ...
  test(`${mode}: mark after interruption reports the failure`, options, async t => {
    const dir = await directory(t);
    const cdp = fake({ fail: true });
    const recorder = createRecorder(cdp, dir);
    await recorder.start();
    ...
    assert.throws(() => recorder.mark('late'), /Simulated disconnection/);
    await assert.rejects(recorder.stop());
  });

  test(`${mode}: complete take is resumably saved and cannot be overwritten`, options, async t => {
    const dir = await directory(t);
    const cdp = fake();
    const recorder = createRecorder(cdp, dir);
    await recorder.start();
    ...
    assert.equal(manifest.frames.length, 1);
    assert(cdp.commands.includes('Page.stopScreencast'));
